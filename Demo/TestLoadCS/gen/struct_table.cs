...
namespace Test{
    ...
    [System.Serializable]
    public sealed partial class TableTTC: Table<TTC>
    {
        public override bool IsMutiLang => false;
        public override string SheetName => "TC";
    

        [System.NonSerialized] public Dictionary<string, TTC> Map;        


        public TTC Find(string Id)
        {
            if (Map == null)
            {
                Log.E("TableTTC's map is null");
                return null;
            }
...
;
            Map.TryGetValue(Id, out var ret);
            ;
            Log.Assert(ret != null, "can't find {0} in TableTTC", Id);
            return ret;
        }    
