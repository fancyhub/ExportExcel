...
local function _LoadItemData()
    ...
    if first_line == nil or #first_line ~= 7 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    local sec_line = csv_line_reader()
    if sec_line == nil or #sec_line ~= 7 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    ...
    for line in csv_line_reader do
		...
		data.PairFieldList= TableUtil.Parse_list_int32_int64(line[6])
		data.ListField= TableUtil.Parse_list_int32(line[7])
