...
local function _LoadItemData()
    ...
    if first_line == nil or #first_line ~= 10 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    local sec_line = csv_line_reader()
    if sec_line == nil or #sec_line ~= 10 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    ...
    for line in csv_line_reader do
		...
		data.Type= TableUtil.Parse_int32(line[2])
		data.SubType= TableUtil.Parse_int32(line[3])
		data.Quality= TableUtil.Parse_int32(line[4])
		data.PairField= TableUtil.Parse_int32_bool(line[5])
		data.PairField2= TableUtil.Parse_int32_bool(line[6])
		data.PairField3= TableUtil.Parse_int32_int32(line[7])
		data.PairFieldList= TableUtil.Parse_list_int32_int64(line[8])
		data.PairFieldList2= TableUtil.Parse_list_int32_int64(line[9])
		data.ListField= TableUtil.Parse_list_int32(line[10])
...
local function _LoadTestComposeKey()
    ...
    local first_line = csv_line_reader()
    if first_line == nil or #first_line ~= 5 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    local sec_line = csv_line_reader()
    if sec_line == nil or #sec_line ~= 5 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    ...
    for line in csv_line_reader do
		...
		data.Name= TableUtil.Parse_locstr(line[3])
		data.Pos= TableUtil.Parse_float32_float32(line[4])
		data.Flags= TableUtil.Parse_int32(line[5])
		table.insert(list_data, data)
