...
local function _LoadItemData()
    ...
    if first_line == nil or #first_line ~= 11 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    local sec_line = csv_line_reader()
    if sec_line == nil or #sec_line ~= 11 then
        log("加载 表格失败, 格式不对 " .. sheet_name)
        return
    end
    ...
    for line in csv_line_reader do
		...
		data.Name= TableUtil.Parse_locstr(line[2])
		...
		data.PairField2= TableUtil.Parse_int32_bool(line[7])
		data.PairField3= TableUtil.Parse_int32_int32(line[8])
		data.PairFieldList= TableUtil.Parse_list_int32_int64(line[9])
		data.PairFieldList2= TableUtil.Parse_list_int32_int64(line[10])
		data.ListField= TableUtil.Parse_list_int32(line[11])
