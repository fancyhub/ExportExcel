...
namespace ExportExcel
{
     ...
     *      
     */
    public class Exporter_DataBinCsv : IProcessNode
    {
        ...
        public Exporter_DataBinCsv(EExportFlag flag, Config.BinConfig config)
        {
            _flag = flag;
            _config = config;
            S_SIGN = 0;
        ...
        public class BinTableRow
        {
            ...
            public void _WriteCell(DataType data_type, string cell, StrDict str_dict)
            {
                if (data_type.IsList)
                {
                    ...
                    {
                        string[] list = cell.Split(ConstDef.SeparatorList);
                        ...
                        if (data_type.IsTuple)
                        {
                            ...
                            {
                                string[] pair = v.Split(ConstDef.SeparatorTuple);
                ...
                else if (data_type.IsTuple)
                {
                    ...
                    else
                    {
                        ...
                        string[] pair = cell.Split(ConstDef.SeparatorTuple);
                        for (int i = 0; i < data_type.Count; i++)
                        {
                            _WriteVal(data_type.Get(i), pair[i], str_dict);
                        }
