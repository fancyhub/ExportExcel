...
{    
    public class Exporter_CppLoaderFromCsv : IProcessNode
    {
        public const string C_HEADER_FILE_NAME = "table_loader_csv.h";
        public const string C_CPP_FILE_NAME = "table_loader_csv.cpp";
        ...
        public Exporter_CppLoaderFromCsv(EExportFlag flag, Config.CppConfig config)
        {
            _flag = flag;
            _config = config;
        }
...
{");
                foreach (FilterTable table in tables)
                {
                    ...
                    sw.WriteLine($"\tLoaderDict[typeid(Table{className})] = TableLoaderInfo(_Load{table.SheetName}, {multi_lang});");
                }
                sw.WriteLine("}");
            }
