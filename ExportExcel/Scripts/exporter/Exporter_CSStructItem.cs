...
namespace ExportExcel
{
    ...
    public class Exporter_CSStructItem : IProcessNode
    {
        public const string C_FILE_NAME = "struct_item.cs";
        public StringFormater _formater = new StringFormater();
        public Config.CSharpConfig _config;
        public EExportFlag _flag;

        public Exporter_CSStructItem(EExportFlag flag, Config.CSharpConfig config)
        {
            _flag = flag;
            _config = config;
        }
    ...
    {".Replace("{enum_name}", p.Key));
            ...
            {
                _formater["parent_class"] = " : " + _config.itemParentClass;
            }
            //生成类结构
